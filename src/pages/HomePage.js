...
const HomePage = () => {
  ...
  const handleDelete = (id, title) => {
    if (window.confirm('Ban co chac muon xoa phim "' + title + '"?')) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.title || !form.poster) {
      setModalError('Vui long nhap ten phim va poster!');
      return;
    }
    const movieData = {
    ...
    if (editing) {
      axios.put('http://localhost:3001/movies/' + editing, movieData)
        ...
        .catch(() => setModalError('Loi khi cap nhat phim!'));
    } else {
      axios.post('http://localhost:3001/movies', movieData)
        .then(() => { fetchMovies(); setShowModal(false); })
        .catch(() => setModalError('Loi khi them phim!'));
    }
  };
  ...
  return (
    <Container className="pb-5">
      ...
      {/* Modal Add/Edit Movie for Admin directly on Home */}
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <div className="row g-3">
              <div className="col-md-6">
                <Form.Group className="mb-3">
                  <Form.Label className="text-light">The loai (vd: Action, Drama)</Form.Label>
