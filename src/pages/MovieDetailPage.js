...
const MovieDetailPage = () => {
  ...
  if (!movie) return <Container className="text-center py-5"><h4 className="text-white">Khong tim thay phim</h4><Button as={Link} to="/" variant="warning" className="mt-3 fw-bold text-dark">Ve trang chu</Button></Container>;

  return (
    <Container className="pb-5">
      <Card className="bg-dark-card border-0 rounded-3 p-3 mb-4">
        <Row className="g-0">
          <Col md={4} className="p-2">
            <img src={movie.poster} alt={movie.title} className="img-fluid rounded-3 w-100" style={{ maxHeight: '480px', objectFit: 'cover' }} />
          </Col>
          <Col md={8} className="p-4 d-flex flex-column justify-content-between">
            <div>
              ...
              <h1 className="fw-bold text-white mb-3">{movie.title}</h1>
            ...
            <div>
              {movie.status === 'now_showing' ? (
                <Button as={Link} to={'/booking/' + movie.id} variant="warning" size="lg" className="px-5 fw-bold text-dark">Dat Ve Ngay</Button>
              ) : (
                <Button variant="secondary" size="lg" disabled className="px-5 fw-bold">Sap Chieu</Button>
              )}
            </div>
          </Col>
        </Row>
      ...
      {movie.trailer && (
        <div className="bg-dark-card p-4 rounded-3">
          <h4 className="fw-bold text-warning mb-3">Trailer</h4>
